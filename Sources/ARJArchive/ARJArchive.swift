...
public struct ARJArchive: Sendable {
    ...
    public func extract(entry: ARJEntry) throws -> Data {
        let parsed = try parsedEntry(for: entry)
        return try extractStoredData(from: parsed)
    }

    public func extract(named name: String) throws -> Data {
    ...
    private func extractStoredData(from parsed: ARJParsedEntry) throws -> Data {
        if parsed.entry.isEncrypted {
            throw ARJError.unsupportedEncryptedArchive
        }

        let compressedData = data.subdata(in: parsed.dataRange)
