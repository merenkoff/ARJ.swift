...
enum ARJDispatch {
    static func run(preprocessed argv: [String]) throws {
        ...
        case "comment":
            ...
            try cmd.run()
        case "add":
            var cmd = try AddStubCommand.parseAsRoot(parseArguments)
            try cmd.run()
        case "delete":
            var cmd = try DeleteStubCommand.parseAsRoot(parseArguments)
            try cmd.run()
        case "update":
    ...
    private static func subcommandHelpText(_ subcommand: String) throws -> String {
        ...
        case "add":
            return AddStubCommand.helpMessage(for: AddStubCommand.self)
        case "delete":
            return DeleteStubCommand.helpMessage(for: DeleteStubCommand.self)
        case "update":
